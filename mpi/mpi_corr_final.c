...
void master(int rank, int P, int N)
{
    ...
    //Transposición + Máximo de A
    globalMax = 0;
    MPI_Bcast(A, N*N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    for (i=0; i<N; i++){
        for (j=0; j<N; j++){
            if (A[i*N+j] > globalMax)
                globalMax = A[i*N+j];
            At[j*N+i] = A[i*N+j];
        }
    }
    ...
    //Multiplicacion (C*D)*maxA y suma de valores CD para calculo promedio
